			...
			<div className="mb-4 grid gap-4">
				...
				<Panel>
					<div className="grid gap-5 lg:grid-cols-2 lg:items-start">
						<ReadonlyWeaponTable weapons={weapons} skillOptions={combatSkills} />
				...
				{inventory.length === 0 ? (
					...
					<div className="grid gap-3">
						{inventory.map((item, index) => (
							<SubPanel
								key={item.id}
								className="grid gap-3 lg:grid-cols-[1.2fr_0.8fr_0.8fr_auto] lg:items-end">
								<TextInput
									label={`物品 ${index + 1}`}
									value={item.name}
...
function WeaponEditor({
	...
	return (
		<SubPanel className="grid gap-4">
			...
			<div className="grid gap-3 lg:grid-cols-[1.25fr_0.8fr_1fr_0.8fr]">
				<TextInput label="武器名称" value={weapon.name} onChange={(event) => onChange("name", event.target.value)} />
				<SelectField label="类型" value={weapon.type} onChange={(value) => onChange("type", value)} options={WEAPON_TYPES.map((type) => ({ value: type, label: type }))} />
				<SelectField label="使用技能" value={weapon.skill} onChange={(value) => onChange("skill", value)} options={skillOptions.map((option) => ({ value: option.label, label: option.label }))} />
			...
			<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
				...
				<TextInput label="次数" type="number" value={weapon.attacksPerRound || ""} onChange={(event) => onChange("attacksPerRound", Number(event.target.value) || 0)} />
				...
				<label className="flex items-center gap-2 self-end border border-border/60 px-3 py-2">
...
function SelectField({
	...
	return (
		<label className="grid gap-1.5">
			...
			<select
				className="h-10 border border-transparent border-b-input bg-transparent px-0 py-2 text-sm outline-none focus-visible:border-b-ring"
				value={value}
