      ...
      {occupation ? (
        <>
          ...
          >
            ...
            <SummaryCard
              label="职业点 / 已分配"
              value={`${occupationSummary.occupationPointsTotal} / ${occupationSummary.occupationPointsSpent}`}
            />
            <SummaryCard
              label="兴趣点 / 已分配"
              value={`${occupationSummary.interestPointsTotal} / ${occupationSummary.interestPointsSpent}`}
            />
          </Box>

          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
            <Chip
              label={`信用评级 ${occupationSummary.creditRatingMin}-${occupationSummary.creditRatingMax}`}
              color={occupationSummary.creditRatingInRange ? "success" : "warning"}
              variant="outlined"
            />
            <Chip label={`当前职业：${info.occupation || "未选择"}`} variant="outlined" />
            <Chip
              label={`剩余职业点 ${Math.max(0, occupationSummary.occupationPointsRemaining)}`}
            ...
            <FormControl key={group.id} fullWidth size="small">
              ...
              <Select
                ...
                renderValue={(selected) =>
                  (selected as string[])
                    .map((optionId) => group.options.find((item) => item.id === optionId)?.label ?? optionId)
                    .join("，")
                }
              >
        ...
        <Typography variant="body2" color="text.secondary">
          先选择职业，再进入技能表分配本职技能点。未选职业时，职业点输入将被锁定。
        </Typography>
      )}
    </Box>
  );
  ...
  return (
    <Box
      sx={{
        ...
        backgroundColor: alpha("#0d1110", 0.48),
      }}
    >
