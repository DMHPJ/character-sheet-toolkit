...
export default function Header({ onOpenInfo }: { onOpenInfo: () => void }) {
  ...
  return (
    <Paper sx={{ p: { xs: 2, md: 3 }, position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, rgba(23,29,27,0.96), rgba(15,20,18,0.96))' }}>
      <Box sx={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at right top, rgba(209,171,87,0.12), transparent 28%)', pointerEvents: 'none' }} />

      <Box sx={{ position: 'relative', display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', lg: 'minmax(280px, 1.3fr) minmax(340px, 1fr) minmax(260px, 0.9fr)' }, alignItems: 'center' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <Avatar
            src={info.portrait || undefined}
            sx={{ width: 88, height: 88, bgcolor: alpha('#3fa17b', 0.22), color: 'primary.main', border: (theme) => `1px solid ${alpha(theme.palette.primary.main, 0.3)}` }}
          >
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            {conditions.length > 0 ? (
              conditions.map((condition) => (
                <Chip key={condition} label={condition} color={condition.includes('疯狂') || condition === '昏迷' ? 'warning' : 'error'} />
              ))
            ) : (
...
function MiniStat({ label, value }: { label: string; value: string | number }) {
  return (
    <Box sx={{ p: 1.25, borderRadius: .5, border: (theme) => `1px solid ${theme.palette.divider}`, backgroundColor: alpha('#0d1110', 0.45) }}>
      <Typography variant="caption" color="text.secondary">
        {label}
      </Typography>
