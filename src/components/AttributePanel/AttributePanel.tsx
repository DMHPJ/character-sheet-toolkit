...
export default function AttributePanel() {
  ...
  const totalPoints = ATTR_META.filter((attribute) => attribute.key !== "Luck").reduce((sum, attribute) => sum + attributes[attribute.key], 0);

  return (
    <Paper sx={{ p: { xs: 2, md: 3 }, backgroundColor: alpha("#171d1b", 0.84) }}>
      <Box sx={{ display: "grid", gap: 2.5 }}>
        <Box sx={{ display: "flex", gap: 1.5, justifyContent: "space-between", flexDirection: { xs: "column", sm: "row" } }}>
        ...
        <Box sx={{ display: "grid", gap: 2, gridTemplateColumns: { xs: "1fr", sm: "repeat(2, minmax(0, 1fr))", lg: "repeat(3, minmax(0, 1fr))" } }}>
          {ATTR_META.map(({ key, label, en }) => {
            ...
            return (
              <Box key={key} sx={{ p: 2, borderRadius: 0.5, border: (theme) => `1px solid ${theme.palette.divider}`, background: "linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01))" }}>
...
function DerivedBadge({ label, value }: { label: string; value: string | number }) {
  return (
    <Box sx={{ borderRadius: 0.5, border: (theme) => `1px solid ${theme.palette.divider}`, px: 1.25, py: 1, backgroundColor: alpha("#0d1110", 0.4) }}>
      <Typography variant="caption" color="text.secondary">
        {label}
      </Typography>
